...
class TestTrailingStopExitSignalStrategy(IStrategy):
    minimal_roi = {"0": 0.1}
    stoploss = -0.3
    timeframe = '5m'
    startup_candle_count = 30

    trail_perc = RealParameter(0.005, 0.08, default=0.02, space="sell")

    def __init__(self, config: dict) -> None:
        super().__init__(config)
        self.exit_signal = TrailingStopExitSignal()
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        trail = self.trail_perc.value
        signal = self.exit_signal.generate(dataframe, metadata["pair"], {"trail_perc": trail})
