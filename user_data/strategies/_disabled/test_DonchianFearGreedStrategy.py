...
class TestDonchianFearGreedStrategy(IStrategy):
    ...
    #  추세 필터 파라미터 (지금은 하이퍼옵트 X, 수동 튜닝용)
    use_trend_filter = IntParameter(0, 1, default=buy_params["use_trend_filter"], space=None, optimize=False)
    trend_ema_period = IntParameter(50, 200, default=buy_params["trend_ema_period"], space=None, optimize=False)
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: Dict[str, Any]) -> pd.DataFrame:
        ...
        #  추세 EMA (레짐 필터용)
        trend_p = int(self._param("trend_ema_period", int))
    ...
    def populate_buy_trend(self, dataframe: pd.DataFrame, metadata: Dict[str, Any]) -> pd.DataFrame:
        ...
        #  추세 필터: EMA 위에서만 진입 (옵션)
        use_trend = int(self._param("use_trend_filter", int)) == 1
        if use_trend and "trend_ema" in df.columns:
            cond_trend = df["close"] > df["trend_ema"]
        else:
            cond_trend = True

        df.loc[:, "buy"] = 0
        df.loc[cond_dc & cond_vol & cond_fg & cond_trend, "buy"] = 1
        return df
