...
# loss-cut override(없으면 생성)
HOST_LC="user_data/overrides/exit_losscut_step1.json"
OV_LC="/freqtrade/user_data/overrides/exit_losscut_step1.json"
if [ ! -f "$HOST_LC" ]; then
  cat > "$HOST_LC" <<'JSON'
{
  "strategy_parameters": {
...
need_cmd docker; need_cmd find; need_cmd comm; need_cmd mktemp; need_cmd grep; need_cmd ls; need_cmd head; need_cmd date
...
with zipfile.ZipFile(zip_path) as z:
    ...
    print("ZIP_CFG: V2.guard_enable =", (sp.get("TestDonchianFearGreedStrategyFG") or {}).get("guard_enable","<missing>"))
    print("ZIP_CFG: use_loss_cut_exit =", sp.get("use_loss_cut_exit","<missing>"))
    v4=sp.get("TDFG_GuardV4") or {}
    print("ZIP_CFG: V4 =", {k:v4.get(k) for k in ["guard_close_pos_min","guard_vol_ratio_min","guard_vol_z_min","guard_rsi14_min","guard_close_confirm"] if k in v4})
...
print("\nEXIT_REASON PNL (by sum_profit_abs):")
...
run_one(){
  ...
  local log="${OUTDIR}/bt_exitAB_${TIMERANGE}_${STRATEGY}_${label}_${TS}.log"
  ...
  if [ -n "$extra" ]; then CMD+=(--config "$extra"); fi
...
echo "START EXIT A/B (loss_cut) : STRATEGY=$STRATEGY TIMERANGE=$TIMERANGE"
run_one "BASELINE" ""
run_one "LOSSCUT_ON" "$OV_LC"
