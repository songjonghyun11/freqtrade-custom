...
# ✅ margin 고정 + range만 스윕
MARGIN_FIXED=0.0020
RANGES=(0.0120 0.0100 0.0080)

TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_range_pct_sweep3_${TIMERANGE}_${TS}.csv"
echo "range_max,zip,trades,profit_abs,dd_usdt,dd_pct,sl_count" > "$CSV"
...
extract_summary_line(){
  local log="$1"
  grep -E '│[[:space:]]*'"$STRATEGY"'[[:space:]]*│' "$log" | tail -n 1 || true
}
...
make_override(){
  local r="$1"
  local tag; tag="$(echo "$r" | sed 's/\./p/g')"
  local f_host="${TMPDIR_HOST}/ov_m${MARGIN_FIXED}_r${tag}.json"
  cat > "$f_host" <<JSON
{
  "strategy_parameters": {
    "TDFG_GuardV4": {
      "guard_breakout_margin_min": ${MARGIN_FIXED},
      "guard_range_pct_max": ${r}
    }
  }
}
JSON
...
run_one(){
  ...
  local tag; tag="$(echo "$r" | sed 's/\./p/g')"
  echo ""
  echo "==== RANGE_${r} (MARGIN=${MARGIN_FIXED} TIMERANGE=$TIMERANGE) ===="

  local ov; ov="$(make_override "$r")"

  local before after
  before="$(mktemp)"; after="$(mktemp)"
  list_zip_names > "$before" || true

  local log="${OUTDIR}/bt_range_${TIMERANGE}_${STRATEGY}_r${tag}_${TS}.log"
  ...
  OUT="user_data/backtest_results/sweep_logs/_slcnt_range_${tag}_${TS}.out"
  ...
  echo "${r},${zip},${trades},${profit_abs},${dd_usdt},${dd_pct},${slc}" >> "$CSV"
}

echo "START: range_pct sweep3 | margin=${MARGIN_FIXED} | ranges=${RANGES[*]}"
echo "CSV=$CSV"
for r in "${RANGES[@]}"; do
  run_one "$r"
done
