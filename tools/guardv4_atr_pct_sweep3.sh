...
# ✅ 현재 best 유지(마진 0.0020 포함하려면 여기 override로 같이 넣어줘야 함)
# 이미 너가 margin을 override로 쓰고 있다면, 아래 OV_MARGIN 파일로 교체해서 추가하면 됨.
# 일단은 "ATR 게이트만" 추가하는 실험이므로 여긴 ATR만 스윕.

ATRS=(0.007 0.009 0.011)
TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_atr_pct_sweep3_${TIMERANGE}_${TS}.csv"
echo "atr_pct_max,zip,trades,profit_abs,dd_usdt,dd_pct,sl_count" > "$CSV"
...
extract_summary_line(){ grep -E '│[[:space:]]*'"$STRATEGY"'[[:space:]]*│' "$1" | tail -n 1 || true; }
...
make_override(){
  local a="$1"
  local tag; tag="$(echo "$a" | sed 's/\./p/g')"
  local f_host="${TMPDIR_HOST}/ov_atr_${tag}.json"
  cat > "$f_host" <<JSON
{ "strategy_parameters": { "TDFG_GuardV4": { "guard_atr_pct_max": ${a} } } }
JSON
...
run_one(){
  ...
  local tag; tag="$(echo "$a" | sed 's/\./p/g')"
  echo ""
  echo "==== ATRPCT_${a} (TIMERANGE=$TIMERANGE) ===="

  local ov; ov="$(make_override "$a")"
  local before after
  before="$(mktemp)"; after="$(mktemp)"
  list_zip_names > "$before" || true

  local log="${OUTDIR}/bt_atr_${TIMERANGE}_${STRATEGY}_${tag}_${TS}.log"
  ...
  OUT="user_data/backtest_results/sweep_logs/_slcnt_atr_${tag}_${TS}.out"
  ...
  echo "${a},${zip},${trades},${profit_abs},${dd_usdt},${dd_pct},${slc}" >> "$CSV"
}

echo "START: ATR% sweep3 | atrs=${ATRS[*]}"
echo "CSV=$CSV"
for a in "${ATRS[@]}"; do
  run_one "$a"
done
