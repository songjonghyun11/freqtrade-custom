...
CSV="$OUTDIR/_exit_ema_grid_v2_${TIMERANGE}_${TS}.csv"
...
make_ema_override() {
  ...
  cat > "$f_host" <<JSON
{ "strategy_parameters": { "exit_fast_ema": ${f}, "exit_slow_ema": ${s} } }
JSON
...
run_case() {
  local f="$1" s="$2"
  # slow <= fast는 스킵(의미 없음/교차 과다)
  if [ "$s" -le "$f" ]; then
    echo "SKIP fast=$f slow=$s (slow<=fast)"
    return 0
  fi
  ...
  local log="${OUTDIR}/bt_exitEmaGridV2_${TIMERANGE}_${STRATEGY}_${tag}_${TS}.log"
  local start_epoch; start_epoch="$(date +%s)"

  set +e
  docker run --rm -v "$(pwd)/user_data":/freqtrade/user_data \
    "$IMG" backtesting --no-color \
    --config "$CONFIG_BASE" --config "$OV_EXIT" --config "$OV_V2OFF" --config "$OV_GUARD" --config "$OV_THR" --config "$ov_ema" \
...
echo "START: exit_fast/slow EMA grid sweep V2 | STRATEGY=$STRATEGY TIMERANGE=$TIMERANGE"
...
for i,row in enumerate(rows[:5],1):
    print(f"{i}) fast={row['fast']} slow={row['slow']} profit_abs={row['profit_abs']} sl_sum={row['sl_sum']} ema_sum={row['ema_sum']} dd={row['dd_usdt']}")
PY
