...
# ✅ 경계 구간만 5점 스윕
THRS=(-0.0065 -0.0070 -0.0075 -0.0080 -0.0085)

TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_ema_loss_thr_focus_${TIMERANGE}_${TS}.csv"
...
make_thr_override() {
  ...
  cat > "$f_host" <<JSON
{ "strategy_parameters": { "ema_exit_loss_thr": ${thr} } }
JSON
...
pick_zip_robust() {
  ...
  if [ -n "${fn:-}" ]; then
    echo "user_data/backtest_results/${fn}"; return 0
  fi
...
run_case() {
  ...
  local log="${OUTDIR}/bt_emaLossThrFocus_${TIMERANGE}_${STRATEGY}_${tag}_${TS}.log"
  local start_epoch; start_epoch="$(date +%s)"

  set +e
  docker run --rm -v "$(pwd)/user_data":/freqtrade/user_data "$IMG" backtesting --no-color \
    --config "$CONFIG_BASE" --config "$OV_EXIT" --config "$OV_V2OFF" --config "$OV_GUARD" --config "$ov_thr" \
    --strategy "$STRATEGY" --strategy-path /freqtrade/user_data/strategies --timerange "$TIMERANGE" > "$log" 2>&1
  local rc=$?
  set -e
  ...
  [ -z "${zip:-}" ] && { echo "ZIP=?"; echo "${thr},?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?" >> "$CSV"; return 0; }
  echo "ZIP=$zip"
...
echo "START: ema_exit_loss_thr focus sweep | STRATEGY=$STRATEGY TIMERANGE=$TIMERANGE"
echo "CSV=$CSV"
echo "THRS=${THRS[*]}"
for thr in "${THRS[@]}"; do run_case "$thr"; done
echo ""
echo "DONE. CSV=$CSV"
"$PY" - <<PY
import csv
...
for i,row in enumerate(rows[:5],1):
    print(f"{i}) thr={row['thr']} profit_abs={row['profit_abs']} sl_sum={row['sl_sum']} ema_sum={row['ema_sum']} dd={row['dd_usdt']}")
PY
