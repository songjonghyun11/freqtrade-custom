...
# 3점만
MARGINS=(0.0015 0.0020 0.0025)

TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_breakout_margin_sweep3_${TIMERANGE}_${TS}.csv"
...
make_margin_override(){
  local m="$1"
  local tag; tag="$(echo "$m" | sed 's/\./p/g')"
  local f_host="${TMPDIR_HOST}/ov_breakout_margin_${tag}.json"
  cat > "$f_host" <<JSON
{ "strategy_parameters": { "TDFG_GuardV4": { "guard_breakout_margin_min": ${m} } } }
JSON
...
run_one(){
  ...
  local tag; tag="$(echo "$m" | sed 's/\./p/g')"
  echo ""
  echo "==== MARGIN_${m} (TIMERANGE=$TIMERANGE) ===="

  local ov; ov="$(make_margin_override "$m")"
  local before after
  before="$(mktemp)"; after="$(mktemp)"
  list_zip_names > "$before" || true

  local log="${OUTDIR}/bt_margin_${TIMERANGE}_${STRATEGY}_${tag}_${TS}.log"
  ...
  # stoploss count only
  OUT="user_data/backtest_results/sweep_logs/_slcnt_${tag}_${TS}.out"
...
echo "START: breakout_margin sweep3 | MARGINS=${MARGINS[*]}"
echo "CSV=$CSV"
for m in "${MARGINS[@]}"; do
  run_one "$m"
done
