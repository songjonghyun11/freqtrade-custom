...
CSV="$OUTDIR/_ema_hold_sweep_${TIMERANGE}_${TS}.csv"
echo "hold_min,zip,trades,profit_abs,dd_usdt,dd_pct,zip_hold,ema_cnt,ema_sum,sl_cnt,sl_sum,roi_cnt,roi_sum,trail_cnt,trail_sum" > "$CSV"
...
make_hold_override() {
  local h="$1"
  local f_host="${TMPDIR_HOST}/ov_ema_hold_${h}.json"
  cat > "$f_host" <<JSON
{ "strategy_parameters": { "ema_exit_min_hold_min": ${h} } }
JSON
...
if not zip_path or not os.path.exists(zip_path):
    print("HOLD=? EMA_CNT=? EMA_SUM=? SL_CNT=? SL_SUM=? ROI_CNT=? ROI_SUM=? TR_CNT=? TR_SUM=?")
    sys.exit(0)
...
with zipfile.ZipFile(zip_path) as z:
    ...
    if cfgs:
        ...
        hold=sp.get("ema_exit_min_hold_min","?")
...
if trades is None:
    print(f"HOLD={hold} EMA_CNT=? EMA_SUM=? SL_CNT=? SL_SUM=? ROI_CNT=? ROI_SUM=? TR_CNT=? TR_SUM=?")
    sys.exit(0)
...
print(f"HOLD={hold} EMA_CNT={ema_cnt} EMA_SUM={ema_sum:.6f} SL_CNT={sl_cnt} SL_SUM={sl_sum:.6f} ROI_CNT={roi_cnt} ROI_SUM={roi_sum:.6f} TR_CNT={tr_cnt} TR_SUM={tr_sum:.6f}")
PY
}
...
run_case() {
  local h="$1"
  echo ""
  echo "==== HOLD_${h} (TIMERANGE=$TIMERANGE) ===="

  local ov_h; ov_h="$(make_hold_override "$h")"

  local before after
  before="$(mktemp)"; after="$(mktemp)"
  list_zip_names > "$before" || true

  local log="${OUTDIR}/bt_emaHold_${TIMERANGE}_${STRATEGY}_h${h}_${TS}.log"
  local start_epoch; start_epoch="$(date +%s)"

  set +e
  docker run --rm -v "$(pwd)/user_data":/freqtrade/user_data \
    "$IMG" backtesting --no-color \
    --config "$CONFIG_BASE" --config "$OV_EXIT" --config "$OV_V2OFF" --config "$OV_GUARD" --config "$OV_BEST" --config "$ov_h" \
  ...
  local zip_hold ema_cnt ema_sum sl_cnt sl_sum roi_cnt roi_sum tr_cnt tr_sum
  zip_hold="$(echo "$es" | sed -n 's/.*HOLD=\([^ ]*\).*/\1/p')"
  ...
  echo "${h},${zip},${trades},${profit_abs},${dd_usdt},${dd_pct},${zip_hold},${ema_cnt},${ema_sum},${sl_cnt},${sl_sum},${roi_cnt},${roi_sum},${tr_cnt},${tr_sum}" >> "$CSV"
}

echo "START: ema_exit_min_hold_min sweep | STRATEGY=$STRATEGY TIMERANGE=$TIMERANGE"
...
for i,row in enumerate(rows[:3],1):
    print(f"{i}) hold={row['hold_min']} profit_abs={row['profit_abs']} sl_sum={row['sl_sum']} ema_sum={row['ema_sum']} dd={row['dd_usdt']}")
PY
