...
# ✅ margin 고정 + wick만 스윕
MARGIN_FIXED=0.0020
WICKS=(0.45 0.55 0.65)

TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_wick_sweep3_${TIMERANGE}_${TS}.csv"
echo "wick_max,zip,trades,profit_abs,dd_usdt,dd_pct,sl_count" > "$CSV"
...
make_override(){
  local w="$1"
  local tag; tag="$(echo "$w" | sed 's/\./p/g')"
  local f_host="${TMPDIR_HOST}/ov_wick_${tag}.json"
  cat > "$f_host" <<JSON
{
  "strategy_parameters": {
    "TDFG_GuardV4": {
      "guard_breakout_margin_min": ${MARGIN_FIXED},
      "guard_upper_wick_ratio_max": ${w}
    }
  }
}
JSON
...
run_one(){
  ...
  local tag; tag="$(echo "$w" | sed 's/\./p/g')"
  echo ""
  echo "==== WICK_${w} (MARGIN=${MARGIN_FIXED} TIMERANGE=$TIMERANGE) ===="

  local ov; ov="$(make_override "$w")"
  local before after
  before="$(mktemp)"; after="$(mktemp)"
  list_zip_names > "$before" || true

  local log="${OUTDIR}/bt_wick_${TIMERANGE}_${STRATEGY}_w${tag}_${TS}.log"
  ...
  OUT="user_data/backtest_results/sweep_logs/_slcnt_wick_${tag}_${TS}.out"
  ...
  echo "${w},${zip},${trades},${profit_abs},${dd_usdt},${dd_pct},${slc}" >> "$CSV"
}

echo "START: wick sweep3 | margin=${MARGIN_FIXED} | wicks=${WICKS[*]}"
echo "CSV=$CSV"
for w in "${WICKS[@]}"; do
  run_one "$w"
done
