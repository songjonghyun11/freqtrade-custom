...
HOLDC=2
MARGINS=(0.0010 0.0015 0.0020)

TMPDIR_HOST="user_data/overrides/sweep_tmp"
mkdir -p "$TMPDIR_HOST"

CSV="$OUTDIR/_hold2_margin_sweep3_${TIMERANGE}_${TS}.csv"
...
extract_summary_line(){ grep -E '│[[:space:]]*'"$STRATEGY"'[[:space:]]*│|[|][[:space:]]*'"$STRATEGY"'[[:space:]]*[|]' "$1" | tail -n 1 || true; }
...
echo "START: hold=${HOLDC} margin sweep3 | margins=${MARGINS[*]}"
echo "CSV=$CSV"
for m in "${MARGINS[@]}"; do
  run_one "$m"
done
